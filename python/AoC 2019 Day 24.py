...
def step(bugs):
  ...
  for row in range(5):
    for col in range(5):
      neighbors = sum(bugs[(row+dr, col+dc)] for dr, dc in [(-1, 0), (1, 0), (0, -1), (0, 1)])
      lives = bugs[(row, col)] and (neighbors == 1)
      infested = not bugs[(row, col)] and neighbors in [1, 2]
      result[(row, col)] = lives or infested
  return result

def solve(bugs):
  seen = set()
  while True:
    biodeversity = sum(pow(2, 5 * row + col) for row in range(5) for col in range(5) if bugs[(row, col)])
    if biodeversity in seen:
      return biodeversity
    seen.add(biodeversity)
    bugs = step(bugs)

bugs = collections.defaultdict(bool, {(row, col): value == '#' for row, line in enumerate(inp.splitlines()) for col, value in enumerate(line)})
...
# grids = collections.defaultdict(lambda: collections.defaultdict(bool), {0: bugs})

# def step_grids(grids):
#   grids[min(grids) + 1] = None
  
#   grids[max(grids) + 1] = None
...
def step_grids(grids):
  grids[min(grids) - 1] = collections.defaultdict(bool)
  grids[max(grids) + 1] = collections.defaultdict(bool)
  for depth, bugs in grids.items():
    pass
